...
bool set_beacon_request_data(uint64_t mac_address, uint8_t src_address_mode, uint8_t data)
{
    ...
    if (is_mac_in_pairing_table(mac_address) != -1)
    {   // we have this device in our pairing table, so set ASSOCIATION_PERMIT
        // Note it might already be set if we are in pairing mode.
        send_beacon=true;
		PAIRING_REQUEST mode = {0,true,PAIRING_REQUEST_SOURCE_BEACON_REQUEST};
//		zprintf(CRITICAL_IMPORTANCE,"BEACON_REQUEST triggering snd_pairing_mode(PAIRING_REQUEST_SOURCE_BEACON_REQUEST)\r\n");
