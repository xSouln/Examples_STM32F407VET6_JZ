...
static void SureFlapDeviceInitHandler(SureFlapDeviceControlT* control)
{
...
static void SureFlapDeviceConversationStartHandler(SureFlapDeviceControlT* control)
{
	while(((control->Devices[control->DeviceHandlerIndex].StatusExtra.AwakeStatus.Mode == SUREFLAP_DEVICE_ASLEEP)
	|| ((get_microseconds_tick()-device_status[current_conversee].last_heard_from) > TIMEOUT_DEVICE_STAYS_AWAKE))
	&& (current_conversee < SUREFLAP_NUMBER_OF_DEVICES))
	{
	...
	else
	{
		...
		if((control->Devices[control->DeviceHandlerIndex].StatusExtra.SendSecurityKey != SUREFLAP_DEVICE_SECURITY_KEY_OK)
		|| (control->Devices[control->DeviceHandlerIndex].StatusExtra.SecureKeyInvalid >= MAX_BAD_KEY))
		{
			...
			{
				SecurityKeyScramble[i] = control->Devices[control->DeviceHandlerIndex].SecurityKey.InByte[i] ^ scramble[i];
			}
...
static void SureFlapDeviceConversationWaitForSecurityAckHandler(SureFlapDeviceControlT* control)
{
...
{
	switch(control->HubOperation)
	{
		case SUREFLAP_HUB_OPERATION_INIT:
			SureFlapDeviceInitHandler(control);
			break;
		...
		case SUREFLAP_HUB_OPERATION_CONVERSATION_START:
			SureFlapDeviceConversationStartHandler(control);
			break;
		...
		case SUREFLAP_HUB_OPERATION_CONVERSATION_WAIT_FOR_SECURITY_ACK:
			SureFlapDeviceConversationWaitForSecurityAckHandler(control);
			break;
