...
#include "ZigbeeSureNetDriver/SureNetDriver.h"
...
void usr_mcps_data_conf(uint8_t msduHandle, uint8_t status) // Called back when a transmit message has gone
{
	...
	SureFlap.Zigbee.TransmissionComplite = true;
		...
		uint8_t status)
{
	...
	if (status == MAC_SUCCESS)
	{
		...
		SureFlapDeviceAcceptPairing(&SureFlap.DeviceControl, DstAddrSpec->Addr.long_address);
...
void usr_mlme_associate_ind(uint64_t DeviceAddress, uint8_t CapabilityInformation, uint8_t dev_type, uint8_t dev_rssi)
{
	...
	if (SureFlapDeviceAssociate(&SureFlap.DeviceControl,
			DeviceAddress,
			dev_type,
			dev_rssi,
...
bool usr_mac_process_beacon_request(uint64_t mac_address, uint8_t src_address_mode, uint8_t data)
{
	...
	if ((data & 0x20) == 0)
	{
		SureFlap.Zigbee.BeaconPayload[BEACON_PAYLOAD_SUREFLAP_VERSION] = HUB_DOES_NOT_SUPPORT_THALAMUS;
		set_beacon_payload(SureFlap.Zigbee.BeaconPayload, true);
	}
	else
	{
		SureFlap.Zigbee.BeaconPayload[BEACON_PAYLOAD_SUREFLAP_VERSION] = HUB_SUPPORTS_THALAMUS;
		set_beacon_payload(SureFlap.Zigbee.BeaconPayload, true);
	}
